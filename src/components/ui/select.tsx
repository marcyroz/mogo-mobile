...
function SelectGroup({
  className,
  ...props
}: SelectPrimitive.GroupProps & React.RefAttributes<SelectPrimitive.GroupRef>) {
  return (
...
function SelectTrigger({
  ...
  return (
    <SelectPrimitive.Trigger
      ref={ref}
      className={cn(
        'border-border dark:bg-input/30 dark:active:bg-input/50 bg-background flex h-12 flex-row items-center justify-between gap-2 rounded-md border px-3 py-2 shadow-sm shadow-black/5 sm:h-9',
        Platform.select({
          web: 'focus-visible:border-ring focus-visible:ring-ring/50 aria-invalid:ring-destructive/20 dark:aria-invalid:ring-destructive/40 aria-invalid:border-destructive dark:hover:bg-input/50 w-fit whitespace-nowrap text-lg outline-none transition-[color,box-shadow] focus-visible:ring-[3px] disabled:cursor-not-allowed [&_svg]:pointer-events-none [&_svg]:shrink-0',
        }),
      ...
      {...props}>
      <>{children}</>
      <Icon as={ChevronDown} aria-hidden={true} className="text-input size-4" />
    </SelectPrimitive.Trigger>
  );
}
...
function SelectItem({
  ...
  return (
    <SelectPrimitive.Item
      ...
      <SelectPrimitive.ItemText className="text-foreground group-active:text-accent-foreground flex-1 select-none text-lg" />
    </SelectPrimitive.Item>
  );
}
